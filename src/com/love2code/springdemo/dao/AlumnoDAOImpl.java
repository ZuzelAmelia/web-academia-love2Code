...
public class AlumnoDAOImpl implements AlumnoDAO {
	...
	public List<Alumno> getAlumnos() {
		...
		Query<Alumno> theQuery = currentSession.createQuery("from Alumno order by primerApellido", Alumno.class);
	...
	public void eliminarAlumno(int elId) {
		...
		// eliminamos de la base de datos el alumno cuyo id coincida con el parámetro
		// elId
		Query theQuery = currentSession.createQuery("delete from Alumno where id=:alumnoId");
	...
	public List<Alumno> buscarAlumnos(String theSearchName) {
		...
		if (theSearchName != null && theSearchName.trim().length() > 0) {
			...
			theQuery = currentSession.createQuery(
					"from Alumno where lower(nombre) like :elNombre or lower(primerApellido) like :elNombre or lower(segundoApellido) like :elNombre",
					Alumno.class);
